...
for step in range(STEPS):
    ...
    if (step+1) % EVAL_INTERNAL == 0:
        ...
        print("=> STEP %10d [TEST]:\trecall:%7.4f \tprecision:%7.4f" %(step+1, test_rec_value,  test_prec_value))
        print("\n=======================> evaluation result <================================\n")


        #tf.summary.scalar("train_recall", train_rec_value)
        #tf.summary.scalar("train_precision", train_prec_value)
        #tf.summary.scalar("test_recall", test_rec_value)
        #tf.summary.scalar("test_precision", test_prec_value)
